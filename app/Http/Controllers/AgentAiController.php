...
class AgentAiController extends Controller
{
    ...
    public function submitFeedback(Request $request, string $token)
    {
        ...
        $learningSession = \App\Models\AiLearningSession::where('chat_session_id', $session->id)
            ->where('ai_response', 'LIKE', '%' . Str::limit($validated['ai_response'], 100) . '%')
            ->orderByDesc('id')
            ->first();

        if ($learningSession) {
            app(\App\Services\AgentAi\AiLearningService::class)->processFeedback(
                $learningSession,
    ...
    public function submitCorrection(Request $request, string $token)
    {
        $validated = $request->validate([
