...
class AgentAiService
{
    public function __construct(
        ...
        private readonly AiMetricService $metricService,
    ...
    public function handleMessage(int $userId, string $promptRaw, ?string $sessionToken = null): array
    {
        ...
        if (!$validScope) {
            ...
            // Enhance with dynamic knowledge
            $dynamicPrompt = $this->learningService->buildDynamicPrompt($systemPrompt, $promptRaw, $session);

            [$assistant, $usage] = $this->llm->chat($session, $setting, $dynamicPrompt);

            $this->saveAssistant($session->id, $assistant, $usage['latency_ms'] ?? 0);
            $this->touchSession($session->id);
            $this->logUsage(false, 'soft_out_of_scope', $setting, $usage);
            $this->metricService->recordInteraction(!empty($assistant), $usage['latency_ms'] ?? 0, $usage['kb_hit'] ?? false);

            // Record learning interaction
            $this->learningService->learnFromInteraction($session->id, $userId, $promptRaw, $assistant);
        ...
        if (in_array($resourceType, ['product', 'service', 'ebook', 'article'], true)) {
            $systemPrompt = $this->promptResolver->resolve([
                'scope' => $scopeCode,
                'intent' => $intentCode,
    ...
    private function saveAssistant(int $sessionId, string $content, int $latencyMs = 0): void
    {
        AiChatMessage::create([
